// App.jsx - Implementación ARIMA Mejorada con PyScript
...
`;
      ...
      // Generar predicción ARIMA en Python
      await generatePythonForecast(processedData);
...
try:
    ...
    if len(data) > 0:
        print(f"Primera fecha: {data['fecha'].min()}")
        print(f"Última fecha: {data['fecha'].max()}")
    
    # Preprocesamiento de datos
    data.set_index('fecha', inplace=True)
    
    # MEJORA: Detectar la frecuencia de los datos
    if len(data) > 1:
        # Calcular la diferencia promedio en días entre fechas consecutivas
        date_diffs = []
        ...
        print(f"Diferencia promedio entre fechas: {avg_diff:.1f} días")
        
        # Determinar la frecuencia basada en la diferencia promedio
        if avg_diff < 2:
            freq = 'D'  # Diario
            print("Frecuencia detectada: Diaria")
        elif 25 <= avg_diff <= 32:
            freq = 'M'  # Mensual
            print("Frecuencia detectada: Mensual")
        elif 6 <= avg_diff <= 8:
            freq = 'W'  # Semanal
            print("Frecuencia detectada: Semanal")
        else:
            freq = 'D'  # Por defecto
            print(f"Frecuencia poco común detectada: {avg_diff:.1f} días. Se usará frecuencia diaria por defecto.")
    else:
        freq = 'D'  # Por defecto con pocos datos
        print("No hay suficientes datos para detectar frecuencia. Se usará frecuencia diaria por defecto.")
    
    # Establecer frecuencia detectada y manejar valores faltantes
    try:
        data = data.asfreq(freq)
        print(f"Serie temporal con frecuencia {freq} establecida correctamente")
    except Exception as e:
        print(f"Error al establecer frecuencia {freq}: {str(e)}")
        print("Intentando con frecuencia diaria por defecto...")
        data = data.asfreq('D')
    ...
    p_value = dftest[1]
    print(f"Prueba de estacionariedad: p-value = {p_value:.4f} (Estacionaria: {p_value <= 0.05})")
    ...
    if p_value > 0.05:
        ...
        use_diff = True
        print("La serie no es estacionaria, se aplicará diferenciación")
    else:
    ...
    # MEJORA: Adaptar los parámetros ARIMA según la frecuencia
    if freq == 'M':
        # Para datos mensuales, posiblemente reducir valores de p y q
        p_adjusted = min(p, 3)  # Limitar p para evitar sobreajuste con datos mensuales
        d_adjusted = d  # Mantener d como está
        q_adjusted = min(q, 2)  # Limitar q para evitar sobreajuste
        print(f"Ajustando parámetros ARIMA para datos mensuales: ({p_adjusted}, {d_adjusted}, {q_adjusted})")
    else:
        # Para datos diarios o de otra frecuencia, usar los parámetros proporcionados
        p_adjusted = p
        d_adjusted = d
        q_adjusted = q
        print(f"Usando parámetros ARIMA proporcionados: ({p_adjusted}, {d_adjusted}, {q_adjusted})")
    
    # Entrenar el modelo ARIMA con los parámetros ajustados
    try:
        print(f"Entrenando modelo ARIMA({p_adjusted}, {d_adjusted}, {q_adjusted})...")
        model = ARIMA(timeseries, order=(p_adjusted, d_adjusted, q_adjusted))
        model_fit = model.fit()
        
        # Realizar predicciones
        print(f"Generando pronóstico para {forecast_days} días...")
        ...
        if use_diff:
            ...
            print(f"Último valor de la serie original: {last_value}")
            ...
            print("Aplicando transformación inversa a la diferenciación")
        else:
            ...
            print("No se requiere transformación inversa")
        
        # MEJORA: Crear índice de fechas según la frecuencia detectada
        if freq == 'M':
            # Para datos mensuales, usar frecuencia mensual en la predicción
            forecast_index = pd.date_range(start=data.index[-1], periods=forecast_days+1, freq='M')[1:]
            print("Generando fechas de pronóstico con frecuencia mensual")
        else:
            # Para otras frecuencias, usar la frecuencia detectada
            forecast_index = pd.date_range(start=data.index[-1], periods=forecast_days+1, freq=freq)[1:]
            print(f"Generando fechas de pronóstico con frecuencia {freq}")
        ...
        print(f"Pronóstico completado: {len(forecast_data)} valores generados")
    except Exception as e:
        print(f"Error en modelo principal: {str(e)}")
        # Si hay un error, intentar con un modelo más simple
        try:
            print("Intentando con un modelo ARIMA más simple...")
            ...
            result = forecast_data
            print(f"Pronóstico con modelo simple completado: {len(forecast_data)} valores generados")
        except Exception as e2:
            print(f"Error en modelo simplificado: {str(e2)}")
            print("Intentando con pronóstico básico basado en tendencia...")
            ...
            try:
                ...
                print(f"Tendencia detectada: {slope:.4f} por período")
                
                # Usar la tendencia detectada para el pronóstico
                last_date = data.index[-1]
                
                # MEJORA: Crear fechas de pronóstico según la frecuencia detectada
                if freq == 'M':
                    forecast_dates = pd.date_range(start=last_date, periods=forecast_days+1, freq='M')[1:]
                else:
                ...
                result = forecast_data
                print(f"Pronóstico basado en tendencia completado: {len(forecast_data)} valores generados")
            except Exception as e3:
                print(f"Error en pronóstico con tendencia: {str(e3)}")
                print("Usando pronóstico más simple con variación aleatoria...")
                ...
                forecast_data = []
                for i in range(forecast_days):
                    # Añadir un pequeño incremento aleatorio al último valor
                ...
                result = forecast_data
                print(f"Pronóstico simple completado: {len(forecast_data)} valores generados")
    ...
    print(f"Se generó un pronóstico de emergencia con {len(fallback_data)} valores")
    ...
    <div className="app-container">
      <header className="app-header">
        <div className="container">
          ...
          <p>Carga tus datos, configura el modelo y genera predicciones de valores absolutos utilizando Python + statsmodels</p>
        </div>
      </header>
          ...
          <div className="panel instruction-panel">
            ...
            <div className="instructions">
              <div className="instruction-item">
                ...
                <p>Sube un archivo Excel (.xlsx o .xls) que contenga al menos una columna de fecha y una columna con los valores que deseas predecir.</p>
              </div>
              <div className="instruction-item">
                <div className="instruction-number">2</div>
                <p>Selecciona la columna que contiene los valores a predecir (valores absolutos, no porcentajes).</p>
              </div>
              <div className="instruction-item">
